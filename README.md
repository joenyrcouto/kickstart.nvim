# 🚀 Obsidian + Neovim: Integração Híbrida
...
Para que a integração funcione, os arquivos deste repositório devem ser distribuídos nas seguintes pastas do seu sistema:

| Arquivo | Destino | Função |
| :--- | :--- | :--- |
| `start-nvim-obsidian.sh` | `~/` (Pasta Home) | Script que limpa a porta 2006 e inicia o servidor Neovim. |
...
Após mover os arquivos, siga estes passos dentro do Obsidian:

### 1. Ativar o Visual (CSS)
1. Vá em `Settings` -> `Appearance`.
2. Role até **CSS Snippets**.
3. Ative o interruptor dos arquivos `*.css`.
   - *Isso removerá as abas do terminal e fará com que ele expanda automaticamente ao ser focado.*

### 2. Configurar o Servidor (Plugin: Terminal)
1. Vá em `Settings` -> `Terminal` -> `Profiles`.
2. No seu perfil (ex: `bridge-nvim`), configure:
   - **Executable:** `/home/SEU_USUARIO/start-nvim-obsidian.sh` (Use o caminho completo).
   - **Arguments:** Deixe a lista vazia.
3. Defina ele como default, na página principal do puglin.
4. Configure o atalho do obsidian para iniciar o terminal com a configuração default para **borda inferior**.

### 3. Configurar o Controle (Plugin: Edit in Neovim)
...
- **Dependências:** O script utiliza `fuser` e `lsof` para gerenciar a porta 2006. Instale-os via pacman:
  ...
  ```
- **Nerd Fonts:** A pseudo-configuração desativa Nerd Fonts no terminal integrado para manter o visual limpo em janelas pequenas. Sua configuração Master (`nvim`) continuará com ícones normais quando aberta fora do Obsidian.
